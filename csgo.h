...
public:
	...
	typedef int(__thiscall* fnSetNextThink) (Player*, int tick);
	using GetSeqDesc_t = void* (__thiscall*)(void*, int);
	using CL_ReadPackets_t = void(__cdecl*)(bool final_tick);
	using SetCollisionBounds_t = void(__thiscall*)(void*, const vec3_t&, const vec3_t&);
	using add_glow_box_t = int(__thiscall*)(void*, vec3_t, ang_t, vec3_t, vec3_t, Color, float);
...
public:
	...
	Address m_uImplPhysicsRunThink;
	fnSetNextThink SetNextThink;
...
public:
	...
	Address					 simulate_player_simulated_entities;
	...
	SetCollisionBounds_t     SetCollisionBounds;
	LoadNamedSky_t           LoadNamedSky;
	CL_ReadPackets_t		 CL_ReadPackets;
	Address                  CacheBoneData;
	Address                  calc_absolute_position;
	Address					 m_AttachmentHelper;
	GetSeqDesc_t			 GetSeqDesc;
...
namespace game {
	...
	__forceinline float GetClientInterpAmount() {
		float flUpdateRate = std::clamp(g_csgo.cl_updaterate->GetFloat(), g_csgo.sv_minupdaterate->GetFloat(), g_csgo.sv_maxupdaterate->GetFloat());
		float flLerpRatio = std::clamp(g_csgo.cl_interp_ratio->GetFloat(), g_csgo.sv_client_min_interp_ratio->GetFloat(), g_csgo.sv_client_max_interp_ratio->GetFloat());
		return std::clamp(flLerpRatio / flUpdateRate, g_csgo.cl_interp->GetFloat(), 1.0f);
	}
	...
	__forceinline bool is_tick_valid(int tick) {
		return tick > 0 && tick <= 17;
	}
	...
	__forceinline int HitboxToHitgroup(int index) {
		...
		case HITBOX_R_FOOT:
		case HITBOX_R_THIGH:
			ret = HITGROUP_RIGHTLEG;
			break;
		case HITBOX_L_CALF:
		case HITBOX_L_FOOT:
		case HITBOX_L_THIGH:
			ret = HITGROUP_LEFTLEG;
			break;
