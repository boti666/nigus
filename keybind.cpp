...
void Keybind::draw() {
	...
	// box outline changes to GUI color if m_set is true, otherwise it's black.
	Color border_color = m_set ? color : Color{ 0, 0, 0, m_parent->m_alpha };
	render::rect(p.x + KEYBIND_X_OFFSET, p.y + 15, m_w - KEYBIND_X_OFFSET, KEYBIND_BOX_HEIGHT, border_color);

	// inner blob
	render::rect_filled(p.x + KEYBIND_X_OFFSET + 1, p.y + 15 + 1, m_w - KEYBIND_X_OFFSET - 2, KEYBIND_BOX_HEIGHT - 2, { 41, 41, 41, m_parent->m_alpha });

	// if m_set is true, display "press a key", otherwise display the assigned key if there is one.
	if (m_set) {
		render::menu.string(p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET, p.y + 15 + 4, color, XOR("press a key"));
	}
