...
void Aimbot::knife() {
	...
	for (const auto& t : m_targets) {
		...
		if (g_lagcomp.StartPrediction(t) != 0) {
			LagRecord* front = t->m_records.front().get();

			if (g_lagcomp.StartPrediction(t) == 1)
				continue;

			if (front->m_pred_origin.dist_to(g_cl.m_shoot_pos) > 512.f)
				continue;

			front->cache();

			// calculate aim angle.
			math::VectorAngles(front->m_pred_origin - g_cl.m_shoot_pos, a);
		...
		// we can history aim.
			...
			if (best->m_pred_origin.dist_to(g_cl.m_shoot_pos) > 512.f)
				continue;
			...
			// calculate aim angle.
			math::VectorAngles(best->m_pred_origin - g_cl.m_shoot_pos, a);
			...
			if (last->m_pred_origin.dist_to(g_cl.m_shoot_pos) > 512.f)
				continue;

			last->cache();

			// calculate aim angle.
			math::VectorAngles(last->m_pred_origin - g_cl.m_shoot_pos, a);
...
bool Aimbot::CanKnife(LagRecord* record, ang_t angle, bool& stab) {
	...
	if (health <= slash_dmg || record->m_pred_origin.dist_to(g_cl.m_local->m_vecOrigin()) > 32.f)
		stab = false;
