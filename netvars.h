...
public:
	...
	void StoreDataMap(Address ptr) {
		...
		for (int i{}; i < map->m_num_fields; ++i) {
			...
			if (!m_offsets[base][var].m_offset) {
				...
				m_offsets[base][var].m_offset = (size_t)entry->m_offset;
	...
	__forceinline uintptr_t FindInDatamap(datamap_t* map, uint32_t name_hash) {
		while (map) {
			for (int i = 0; i < map->m_num_fields; i++) {
				if (!map->m_desc[i].m_name)
					continue;

				if (FNV1a::get(map->m_desc[i].m_name) == name_hash)
					return map->m_desc[i].m_offset;

				if (map->m_desc[i].m_type == FIELD_EMBEDDED && map->m_desc[i].m_td)
					if (const uintptr_t offset = FindInDatamap(map->m_desc[i].m_td, name_hash); offset != 0)
						return offset;
			}
			map = map->m_base;
		}
