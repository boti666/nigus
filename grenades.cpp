...
void Grenades::paint() {
	...
	for (int i{ 1 }; i <= g_csgo.m_globals->m_max_clients; ++i) {
		...
		if (m_id == HEGRENADE) {
			...
			// scale damage.
			g_auto_wall.get()->scale_dmg(player, trace, g_cl.m_weapon_info, damage, HITGROUP_CHEST);
...
void Grenades::setup() {
	...
	auto lel = g_cl.m_local->GetAbsOrigin() + g_cl.m_local->m_vecViewOffset();
	m_start = g_cl.m_local->GetAbsOrigin() + g_cl.m_local->m_vecViewOffset();
