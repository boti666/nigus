...
class AmogusCharacter {
public:
	int x, y; 

	AmogusCharacter(int screenWidth, int screenHeight) {
		static std::default_random_engine generator(std::chrono::system_clock::now().time_since_epoch().count());
		static std::uniform_int_distribution<int> distributionX(0, screenWidth);
		static std::uniform_int_distribution<int> distributionY(0, screenHeight);
...
Color getRainbowColor(float time, float speed) {
	int r = sin(speed * time + 0) * 127 + 128;
	int g = sin(speed * time + 2) * 127 + 128;
	int b = sin(speed * time + 4) * 127 + 128;

	return Color(r, g, b, 255);
}

std::vector<AmogusCharacter> iamdepreset;
bool menuopen = false;

auto startTime = std::chrono::high_resolution_clock::now();

// Then, within your draw function, calculate the elapsed time in seconds
auto currentTimePoint = std::chrono::high_resolution_clock::now();
float currentTime = std::chrono::duration<float>(currentTimePoint - startTime).count();
