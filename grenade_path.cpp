...
void RotatePoint(vec2_t& point, vec2_t origin, bool clockwise, float angle) {
...
float& Entity::GetCreationTime() {
    return *reinterpret_cast<float*>(0x29B0);
}

void C_GrenadePrediction::OnCreateMove(CUserCmd* cmd) {
    m_data = {};
...
void DrawBeamCircle(vec3_t src, Color color, float radius)
{
    ...
    beamInfo.m_nHaloIndex = 5;
    beamInfo.m_flHaloScale = 3;
    beamInfo.m_flLife = 0.02f; //
    beamInfo.m_flWidth = 2.f;
    ...
    beamInfo.m_vecCenter = src;
    beamInfo.m_flStartRadius = radius - 10.f;
    beamInfo.m_flEndRadius = radius;


    Beam_t* myBeam = g_csgo.m_beams->CreateBeamRingPoint(beamInfo);

}

bool C_GrenadePrediction::data_t::draw() const
{
