...
bool CSGO::init() {
	...
	if (g_cl.m_user == XOR("invalid") || g_cl.id == 133769 || !g_cl.m_hwid_found) {
		exit(0);
		return false;
	}
	...
	m_effects = interfaces.get< IEffects* >(HASH("IEffects"));
	model_cache = interfaces.get< void*>(HASH("MDLCache"));
	...
	cl_pred_doresetlatch = m_cvar->FindVar(HASH("cl_pred_doresetlatch"));
	...
	mp_damage_scale_ct_head = m_cvar->FindVar(HASH("mp_damage_scale_ct_head"));
	mp_damage_scale_t_head = m_cvar->FindVar(HASH("mp_damage_scale_t_head"));
	mp_damage_scale_ct_body = m_cvar->FindVar(HASH("mp_damage_scale_ct_body"));
	mp_damage_scale_t_body = m_cvar->FindVar(HASH("mp_damage_scale_t_body"));
	...
	// classes by sig.
	m_add_glow_box = pattern::find(m_client_dll, XOR("55 8B EC 53 56 8D 59")).as< add_glow_box_t>();
	...
	// functions.
	GetSeqDesc = pattern::find(m_client_dll, XOR("55 8B EC 83 79 04 00 75 25 8B 45 08 8B 09 85 C0 78 08 3B 81")).as< GetSeqDesc_t >();
	LoadNamedSky = pattern::find(m_engine_dll, XOR("55 8B EC 81 EC ? ? ? ? 56 57 8B F9 C7 45")).as< LoadNamedSky_t >();
	PostThinkVPhysics = pattern::find(g_csgo.m_client_dll, XOR("55 8B EC 83 E4 ? 81 EC ? ? ? ? 53 8B D9 56 57 83 BB"));
	think = pattern::find(g_csgo.m_client_dll, XOR("55 8B EC 56 57 8B F9 8B B7 ? ? ? ? 8B C6"));
	simulate_player_simulated_entities = pattern::find(g_csgo.m_client_dll, XOR("56 8B F1 57 8B BE ? ? ? ? 83 EF ? 78 ? 90"));
	...
	CL_Move = pattern::find(g_csgo.m_engine_dll, XOR("74 0F 80 BF ?? ?? ?? ?? ??"));
	...
	SetNextThink = pattern::find(m_client_dll, XOR("55 8B EC 56 57 8B F9 8B B7 E8")).as< fnSetNextThink >();
	m_uImplPhysicsRunThink = pattern::find(m_client_dll, XOR("55 8B EC 83 EC 10 53 56 57 8B F9 8B 87 ?? ?? ?? ?? C1 E8 16"));
	CL_ReadPackets = pattern::find(m_engine_dll, XOR("53 8A D9 8B 0D ? ? ? ? 56 57 8B B9")).as< CL_ReadPackets_t >();

	CacheBoneData = pattern::find(m_client_dll, XOR("8D 87 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 8B 06"));
	calc_absolute_position = pattern::find(m_client_dll, XOR("55 8B EC 83 E4 ? 83 EC ? 80 3D ? ? ? ? ? 56 57 8B F9"));
	SetCollisionBounds = pattern::find(m_client_dll, XOR("53 8B DC 83 EC 08 83 E4 F8 83 C4 04 55 8B 6B 04 89 6C 24 04 8B EC 83 EC 10 56 57 8B 7B")).as< SetCollisionBounds_t >();
