...
void math::change_matrix_position(matrix3x4_t* bones, size_t msize, vec3_t current_position, vec3_t new_position)
{
...
float math::normalize(float ang) {
    ang = std::clamp(remainderf(ang, 360.f), -180.f, 180.f);
    return ang;
}


void math::angle_to_vectors(vec3_t angles, vec3_t& forward) {
    float sp, sy, cp, cy;
...
vec3_t math::VectorAmogus(const vec3_t& transform, const matrix3x4_t& matrix) {
    vec3_t out = {
        transform.dot(vec3_t(matrix[0][0], matrix[0][1], matrix[0][2])) + matrix[0][3],
        transform.dot(vec3_t(matrix[1][0], matrix[1][1], matrix[1][2])) + matrix[1][3],
        transform.dot(vec3_t(matrix[2][0], matrix[2][1], matrix[2][2])) + matrix[2][3]
    };
...
float math::DistanceToRay(const vec3_t& pos, const vec3_t& rayStart, const vec3_t& rayEnd, float* along, vec3_t* pointOnRay) {
...
float math::DistanceBetweenSegments(vec3_t s1, vec3_t s2, vec3_t k1, vec3_t k2) {
    static auto constexpr SMALL_NUM = 0.00000001f;
...
bool math::DoesIntersectCapsule(vec3_t eyePos, vec3_t end, vec3_t capsuleA, vec3_t capsuleB, float radius) {
    auto dist = DistanceBetweenSegments(eyePos, end, capsuleA, capsuleB);
...
void math::SetMatrixPosition(vec3_t pos, matrix3x4_t& matrix) {
    for (size_t i{ }; i < 3; ++i) {
        matrix[i][3] = pos[i];
    }
}

vec3_t math::GetMatrixPosition(const matrix3x4_t& src) {
