...
void Config::init() {
	...
	// append our subdir.
	g_winapi.PathAppendA((char*)m_path.c_str(), "C:\\nigus");
...
void Config::LoadHotkeys() {
	...
	g_menu.main.misc.key7.set(config[g_menu.main.misc.key7.m_file_id]);
	g_menu.main.misc.key8.set(config[g_menu.main.misc.key8.m_file_id]);
	g_menu.main.misc.key9.set(config[g_menu.main.misc.key9.m_file_id]);
	g_menu.main.misc.key10.set(config[g_menu.main.misc.key10.m_file_id]);
}
...
void Config::SaveHotkeys() {
	...
	config[g_menu.main.misc.key7.m_file_id] = g_menu.main.misc.key7.get();
	config[g_menu.main.misc.key8.m_file_id] = g_menu.main.misc.key8.get();
	config[g_menu.main.misc.key9.m_file_id] = g_menu.main.misc.key9.get();
	config[g_menu.main.misc.key10.m_file_id] = g_menu.main.misc.key10.get();
...
void Config::load(const Form* form, const std::string& name, bool skins) {
	if (!m_init)
		return;
	...
	for (nlohmann::json::iterator t = config.begin(); t != config.end(); t++) {
		...
		if (skins) { // only load for skins tab 
			if (strcmp(title.c_str(), "skins"))
				continue;
		}
		else {
			if (!strcmp(title.c_str(), "skins"))
...
void Config::save(const Form* form, const std::string& name, bool skins) {
	nlohmann::json config{};
	...
	for (const auto t : form->m_tabs) {
		if (!strcmp(t->m_title.c_str(), "debug"))
			continue;

		if (skins) {
			if (strcmp(t->m_title.c_str(), "skins"))
				continue;
