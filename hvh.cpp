...
void HVH::GetAntiAimDirection() {
	...
	if (!g_hvh.m_left && !g_hvh.m_right && !g_hvh.m_back && !g_hvh.m_forward) {
		...
		if (!g_cl.m_lag && g_csgo.m_globals->m_curtime >= g_cl.m_body_pred && (stand || air)) {
			...
			if (stand) {
				switch (g_menu.main.antiaim.body_fake_stand.get()) {
					...
					// left.
				case 1:
					g_cl.m_cmd->m_view_angles.y += g_cl.m_flick_since_moving == 1 ? -125.f : 75.f;
					break;
	...
	if (g_menu.main.antiaim.lag_enable.get()) {
		...
		if (active) {
			...
			// random.
				...
				if (g_cl.m_local->m_vecVelocity().length_2d() < 90.f || (g_cl.m_lagcomp && g_cl.m_old_lagcomp) || !g_input.GetKeyState(g_menu.main.movement.fake_walk.get()))
					limit_ = 14;

				*g_cl.m_packet = g_cl.m_lag >= limit_;
			}
		}
